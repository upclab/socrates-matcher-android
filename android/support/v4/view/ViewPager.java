...
public class ViewPager extends ViewGroup {
    ...
    static class C00101 implements Comparator<ItemInfo> {
        C00101() {
        }
    ...
    static class C00112 implements Interpolator {
        C00112() {
        }

        public float getInterpolation(float t) {
            t -= AugmentedReality.ONE_PERCENT;
            return ((((t * t) * t) * t) * t) + AugmentedReality.ONE_PERCENT;
        }
    }
    ...
    public static class SavedState extends BaseSavedState {
        ...
        static class C04321 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C04321() {
            }
        ...
        static {
            CREATOR = ParcelableCompat.newCreator(new C04321());
        }
    ...
    static {
        ...
        LAYOUT_ATTRS = iArr;
        COMPARATOR = new C00101();
        sInterpolator = new C00112();
    }
    ...
    void smoothScrollTo(int x, int y, int velocity) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(AugmentedReality.ONE_PERCENT, (AugmentedReality.ONE_PERCENT * ((float) Math.abs(dx))) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(1000.0f * Math.abs(distance / ((float) velocity))) * 4;
        } else {
            duration = (int) ((AugmentedReality.ONE_PERCENT + (((float) Math.abs(dx)) / ((float) (this.mPageMargin + width)))) * AugmentedReality.MAX_ZOOM);
        }
                ...
                switch (action & MotionEventCompat.ACTION_MASK) {
                    ...
                    case SCROLL_STATE_DRAGGING /*1*/:
                        if (this.mIsBeingDragged) {
                            ...
                            velocityTracker.computeCurrentVelocity(DateTimeConstants.MILLIS_PER_SECOND, (float) this.mMaximumVelocity);
