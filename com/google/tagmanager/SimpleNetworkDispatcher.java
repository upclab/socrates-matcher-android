...
class SimpleNetworkDispatcher implements Dispatcher {
    ...
    SimpleNetworkDispatcher(HttpClient httpClient, Context ctx, DispatchListener dispatchListener) {
        ...
        this.userAgent = createUserAgentString("GoogleTagManager", "3.0", VERSION.RELEASE, getUserAgentLanguage(Locale.getDefault()), Build.MODEL, Build.ID);
    ...
    public void dispatchHits(List<Hit> hits) {
        ...
        for (int i = 0; i < maxHits; i++) {
            ...
            URL url = getUrl(hit);
            if (url == null) {
                Log.m612w("No destination: discarding hit.");
                ...
                if (request == null) {
                    ...
                    if (firstSend) {
                        try {
                            ...
                            Log.m612w("ClientProtocolException sending hit; discarding hit...");
                            this.dispatchListener.onHitPermanentDispatchFailure(hit);
                        } catch (IOException e2) {
                            Log.m612w("Exception sending hit: " + e2.getClass().getSimpleName());
                            Log.m612w(e2.getMessage());
                    ...
                    if (statusCode != 200) {
                        Log.m612w("Bad response: " + response.getStatusLine().getStatusCode());
                        this.dispatchListener.onHitTransientDispatchFailure(hit);
                    } else {
    ...
    private HttpEntityEnclosingRequest constructGtmRequest(URL url) {
        ...
        try {
            ...
            try {
                request2.addHeader(HttpHeaders.USER_AGENT, this.userAgent);
                return request2;
            } catch (URISyntaxException e2) {
                e = e2;
                request = request2;
                Log.m612w("Exception sending hit: " + e.getClass().getSimpleName());
                Log.m612w(e.getMessage());
                return request;
            }
    ...
    private void logDebugInformation(HttpEntityEnclosingRequest request) {
        ...
        if (request.getEntity() != null) {
            try {
                ...
                Log.m610v("Error Writing hit to log...");
            }
        }
        Log.m610v(httpHeaders.toString());
    }
    ...
    URL getUrl(Hit hit) {
        try {
            ...
            Log.m606e("Error trying to parse the GTM url.");
            return null;
        }
    }
}
