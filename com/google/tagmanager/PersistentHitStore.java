...
class PersistentHitStore implements HitStore {
    ...
    class UrlDatabaseHelper extends SQLiteOpenHelper {
        ...
        private boolean tablePresent(String table, SQLiteDatabase db) {
            ...
            try {
                ...
                Log.m612w("Error querying for table " + table);
                return false;
            } finally {
                if (cursor != null) {
                    cursor.close();
        ...
        public SQLiteDatabase getWritableDatabase() {
            if (!this.mBadDatabase || this.mLastDatabaseCheckTime + DateUtils.MILLIS_PER_HOUR <= PersistentHitStore.this.mClock.currentTimeMillis()) {
    ...
    class StoreDispatchListener implements DispatchListener {
        ...
        public void onHitPermanentDispatchFailure(Hit hit) {
            PersistentHitStore.this.deleteHit(hit.getHitId());
            Log.m610v("Permanent failure dispatching hitId: " + hit.getHitId());
        }
        ...
        public void onHitTransientDispatchFailure(Hit hit) {
            ...
            if (firstDispatchTime == 0) {
                ...
                PersistentHitStore.this.deleteHit(hit.getHitId());
                Log.m610v("Giving up on failed hitId: " + hit.getHitId());
            }
        }
    }
    ...
    private void removeOldHitIfFull() {
        ...
        if (hitsOverLimit > 0) {
            ...
            Log.m610v("Store full, deleting " + hitsToDelete.size() + " hits to make room.");
    ...
    List<String> peekHitIds(int maxHits) {
        ...
        if (maxHits <= 0) {
            Log.m612w("Invalid maxHits specified. Skipping");
        } else {
            ...
            if (db != null) {
                ...
                try {
                    ...
                    Log.m612w("Error in peekHits fetching hitIds: " + e.getMessage());
                    if (cursor != null) {
                        cursor.close();
                    }
                } catch (Throwable th) {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    if (cursor.moveToFirst()) {
                        do {
                            if (((SQLiteCursor) cursor).getWindow().getNumRows() > 0) {
                                ...
                                Log.m612w(String.format("HitString for hitId %d too large.  Hit will be deleted.", new Object[]{Long.valueOf(((Hit) hits2.get(count)).getHitId())}));
                            }
                            count++;
                        } while (cursor.moveToNext());
                    }
                    ...
                    Log.m612w("Error in peekHits fetching hit url: " + e2.getMessage());
                ...
                hits = hits2;
                try {
                    Log.m612w("Error in peekHits fetching hitIds: " + e2.getMessage());
                    if (cursor != null) {
                        cursor.close();
                    }
                    return hits;
    ...
    private void setHitFirstDispatchTime(long hitId, long firstDispatchTime) {
        ...
        if (db != null) {
            ...
            try {
                ...
                Log.m612w("Error setting HIT_FIRST_DISPATCH_TIME for hitId: " + hitId);
                deleteHit(hitId);
            }
        }
    }
    ...
    int getNumStoredHits() {
        ...
        try {
            ...
            Log.m612w("Error getting numStoredHits");
            if (cursor != null) {
                cursor.close();
            }
        } catch (Throwable th) {
    ...
    int getNumStoredUntriedHits() {
        ...
        try {
            ...
            Log.m612w("Error getting num untried hits");
            if (cursor != null) {
                cursor.close();
            }
        } catch (Throwable th) {
    ...
    public void dispatch() {
        Log.m610v("GTM Dispatch running...");
        if (this.mDispatcher.okToDispatch()) {
            List<Hit> hits = peekHits(40);
            if (hits.isEmpty()) {
                Log.m610v("...nothing to dispatch");
    ...
    public void close() {
        try {
            ...
            Log.m612w("Error opening database for close");
        }
    }
