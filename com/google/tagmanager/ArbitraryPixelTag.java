...
class ArbitraryPixelTag extends TrackingTag {
    ...
    class C08361 implements HitSenderProvider {
        final /* synthetic */ Context val$context;

        C08361(Context context) {
    ...
    public ArbitraryPixelTag(Context context) {
        this(context, new C08361(context));
    }
                ...
                if (l instanceof List) {
                    for (Map<Object, Object> m : l) {
                        if (m instanceof Map) {
                            ...
                            Log.m606e("ArbitraryPixel: additional params contains non-map: not sending partial hit: " + uriBuilder.build().toString());
                            return;
                        }
                    }
                }
                Log.m606e("ArbitraryPixel: additional params not a list: not sending partial hit: " + uriBuilder.build().toString());
                return;
            }
            String uri = uriBuilder.build().toString();
            this.mHitSenderProvider.get().sendHit(uri);
            Log.m610v("ArbitraryPixel: url = " + uri);
