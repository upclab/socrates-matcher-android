...
public class Container {
    ...
    class C08392 implements LoadCallback<ResourceWithMetadata> {
        ...
        public void onSuccess(ResourceWithMetadata resource) {
            if (Container.this.isDefault()) {
                ...
                Log.m610v("setting refresh time to saved time: " + resource.getTimeStamp());
        ...
        private RefreshFailure failureToRefreshFailure(Failure failure) {
            switch (C03284.$SwitchMap$com$google$tagmanager$LoadCallback$Failure[failure.ordinal()]) {
    ...
    class C08403 implements LoadCallback<Resource> {
        final /* synthetic */ Clock val$clock;
        ...
        public void onSuccess(Resource resource) {
            synchronized (Container.this) {
                ...
                Log.m610v("setting refresh time to current time: " + Container.this.mLastRefreshTime);
    ...
    Container(Context context, String containerId, TagManager tagManager, ResourceStorage resourceStorage) {
        this.mResourceVersion = StringUtils.EMPTY;
    ...
    public boolean getBoolean(String key) {
        ...
        if (runtime == null) {
            Log.m606e("getBoolean called for closed container.");
        ...
        try {
            ...
            Log.m606e("Calling getBoolean() threw an exception: " + e.getMessage() + " Returning default value.");
            return Types.getDefaultBoolean().booleanValue();
        }
    }
    ...
    public double getDouble(String key) {
        Runtime runtime = getRuntime();
        if (runtime == null) {
            Log.m606e("getDouble called for closed container.");
        ...
        try {
            ...
            Log.m606e("Calling getDouble() threw an exception: " + e.getMessage() + " Returning default value.");
            return Types.getDefaultDouble().doubleValue();
        }
    }
    ...
    public long getLong(String key) {
        Runtime runtime = getRuntime();
        if (runtime == null) {
            Log.m606e("getLong called for closed container.");
        ...
        try {
            ...
            Log.m606e("Calling getLong() threw an exception: " + e.getMessage() + " Returning default value.");
            return Types.getDefaultInt64().longValue();
        }
    }
    ...
    public String getString(String key) {
        Runtime runtime = getRuntime();
        if (runtime == null) {
            Log.m606e("getString called for closed container.");
            return Types.getDefaultString();
        }
        try {
            return Types.valueToString((TypeSystem.Value) runtime.evaluateMacroReference(key).getObject());
        } catch (Exception e) {
            Log.m606e("Calling getString() threw an exception: " + e.getMessage() + " Returning default value.");
            return Types.getDefaultString();
        }
    }
    ...
    public synchronized void refresh() {
        if (getRuntime() == null) {
            Log.m612w("refresh called for closed container");
        } else {
            try {
                if (isDefaultContainerRefreshMode()) {
                    Log.m612w("Container is in DEFAULT_CONTAINER mode. Refresh request is ignored.");
                } else {
                    long currentTime = this.mClock.currentTimeMillis();
                    if (useAvailableToken(currentTime)) {
                        Log.m610v("Container refresh requested");
                        loadAfterDelay(0);
                        this.mLastRefreshMethodCalledTime = currentTime;
                    } else {
                        Log.m610v("Container refresh was called too often. Ignored.");
                    }
                }
            } catch (Exception e) {
                Log.m606e("Calling refresh() throws an exception: " + e.getMessage());
            }
        }
    }
    ...
    public synchronized void close() {
        try {
            ...
            Log.m606e("Calling close() threw an exception: " + e.getMessage());
        }
        this.mRuntime = null;
    }
    ...
    void load(Callback callback) {
        load(callback, new ResourceLoaderSchedulerImpl(this.mContext, this.mContainerId, this.mCtfeHost), new C08381());
    }
    ...
    private synchronized void callRefreshSuccess(RefreshType refreshType) {
        Log.m610v("calling containerRefreshSuccess(" + refreshType + "): mUserCallback = " + this.mUserCallback);
    ...
    synchronized void load(Callback callback, ResourceLoaderScheduler scheduler, Clock clock) {
        ...
        this.mDiskLoadCallback = new C08392(clock);
        if (isDefaultContainerRefreshMode()) {
            Log.m608i("Container is in DEFAULT_CONTAINER mode. Use default container.");
        } else {
            this.mResourceStorage.setLoadCallback(this.mDiskLoadCallback);
            this.mNetworkLoadCallback = new C08403(clock);
    ...
    private void initEvaluators(Resource resource, boolean isDefaultResource) {
        try {
            ...
            Log.m606e("Not loading resource: " + resource + " because it is invalid: " + err.toString());
        }
    }
    ...
    private void createInitialContainer() {
        ...
        if (expandedResource == null) {
            Log.m612w("No default container found; creating an empty container.");
