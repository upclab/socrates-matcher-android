...
public final class InetAddresses {
    ...
    private static Inet4Address getInet4Address(byte[] bytes) {
        Preconditions.checkArgument(bytes.length == IPV4_PART_COUNT, "Byte array has invalid length for an IPv4 address: %s != 4.", Integer.valueOf(bytes.length));
    ...
    private static String convertDottedQuadToHex(String ipString) {
        ...
        String penultimate = Integer.toHexString(((quad[0] & MotionEventCompat.ACTION_MASK) << IPV6_PART_COUNT) | (quad[1] & MotionEventCompat.ACTION_MASK));
        return initialPart + penultimate + ":" + Integer.toHexString(((quad[2] & MotionEventCompat.ACTION_MASK) << IPV6_PART_COUNT) | (quad[3] & MotionEventCompat.ACTION_MASK));
    }

    private static byte parseOctet(String ipPart) {
        int octet = Integer.parseInt(ipPart);
        if (octet <= MotionEventCompat.ACTION_MASK && (!ipPart.startsWith("0") || ipPart.length() <= 1)) {
            return (byte) octet;
        }
    ...
    public static TeredoInfo getTeredoInfo(Inet6Address ip) {
        ...
        byte[] bytes = ip.getAddress();
        Inet4Address server = getInet4Address(Arrays.copyOfRange(bytes, IPV4_PART_COUNT, IPV6_PART_COUNT));
