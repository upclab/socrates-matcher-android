...
class GAServiceProxy implements ServiceProxy, OnConnectedListener, OnConnectionFailedListener {
    ...
    static /* synthetic */ class C00873 {
        ...
        static {
            ...
            try {
                f0x3c5f147c[ConnectState.CONNECTING.ordinal()] = 3;
            } catch (NoSuchFieldError e3) {
            }
            try {
                f0x3c5f147c[ConnectState.PENDING_CONNECTION.ordinal()] = 4;
            } catch (NoSuchFieldError e4) {
            }
            try {
                f0x3c5f147c[ConnectState.PENDING_DISCONNECT.ordinal()] = 5;
            } catch (NoSuchFieldError e5) {
            }
            try {
                f0x3c5f147c[ConnectState.DISCONNECTED.ordinal()] = 6;
    ...
    private class DisconnectCheckTask extends TimerTask {
        ...
        public void run() {
            if (GAServiceProxy.this.state == ConnectState.CONNECTED_SERVICE && GAServiceProxy.this.queue.isEmpty() && GAServiceProxy.this.lastRequestTime + GAServiceProxy.this.idleTimeout < GAServiceProxy.this.clock.currentTimeMillis()) {
                Log.m3v("Disconnecting due to inactivity");
    ...
    public synchronized void setForceLocalDispatch() {
        if (!this.forceLocalDispatch) {
            Log.m3v("setForceLocalDispatch called.");
    ...
    L_0x0028:
        r0 = com.google.analytics.tracking.android.GAServiceProxy.C00873.f0x3c5f147c;	 Catch:{ all -> 0x0072 }
    ...
    L_0x003e:
        ...
        com.google.analytics.tracking.android.Log.m3v(r0);	 Catch:{ all -> 0x0072 }
    ...
    private synchronized void useStore() {
        if (this.state != ConnectState.CONNECTED_LOCAL) {
            clearAllTimers();
            Log.m3v("falling back to local store");
    ...
    private synchronized void connectToService() {
        if (this.forceLocalDispatch || this.client == null || this.state == ConnectState.CONNECTED_LOCAL) {
            Log.m4w("client not initialized.");
            useStore();
        } else {
            try {
                this.connectTries++;
                ...
                Log.m3v("connecting to Analytics service");
                this.client.connect();
            } catch (SecurityException e) {
                Log.m4w("security exception on connectToService");
                useStore();
            }
        }
    }
    ...
    public synchronized void onDisconnected() {
        if (this.state == ConnectState.PENDING_DISCONNECT) {
            Log.m3v("Disconnected from service");
            clearAllTimers();
            this.state = ConnectState.DISCONNECTED;
        } else {
            Log.m3v("Unexpected disconnect.");
    ...
    public synchronized void onConnectionFailed(int errorCode, Intent resolution) {
        this.state = ConnectState.PENDING_CONNECTION;
        if (this.connectTries < MAX_TRIES) {
            Log.m4w("Service unavailable (code=" + errorCode + "), will retry.");
            fireReconnectAttempt();
        } else {
            Log.m4w("Service unavailable (code=" + errorCode + "), using local store.");
            useStore();
        }
    }
