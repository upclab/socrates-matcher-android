...
class SimpleNetworkDispatcher implements Dispatcher {
    ...
    public boolean okToDispatch() {
        ...
        Log.m3v("...no network connectivity");
        return false;
    }
    ...
    public int dispatchHits(List<Hit> hits) {
        ...
        for (int i = 0; i < maxHits; i++) {
            ...
            if (url == null) {
                if (Log.isVerbose()) {
                    Log.m4w("No destination: discarding hit: " + hit.getHitParams());
                } else {
                    Log.m4w("No destination: discarding hit.");
                }
                hitsDispatched++;
            } else {
                String params;
                ...
                if (request == null) {
                    ...
                    if (params.length() > 8192) {
                        Log.m4w("Hit too long (> 8192 bytes)--not sent");
                    } else {
                        if (this.gaInstance.isDryRunEnabled()) {
                            Log.m2i("Dry run enabled. Hit not actually sent.");
                        } else {
                            if (firstSend) {
                                try {
                                    ...
                                    Log.m4w("ClientProtocolException sending hit; discarding hit...");
                                } catch (IOException e2) {
                                    Log.m4w("Exception sending hit: " + e2.getClass().getSimpleName());
                                    Log.m4w(e2.getMessage());
                                }
                            }
                            ...
                            if (statusCode != 200) {
                                Log.m4w("Bad response: " + response.getStatusLine().getStatusCode());
                            }
                        }
                    }
                    hitsDispatched++;
    ...
    private HttpEntityEnclosingRequest buildRequest(String params, String path) {
        ...
        if (full.length() < 2036) {
            ...
            try {
                ...
                Log.m4w("Encoding error, discarding hit");
                return null;
            }
        }
    ...
    private void logDebugInformation(HttpEntityEnclosingRequest request) {
        ...
        if (request.getEntity() != null) {
            try {
                ...
                Log.m3v("Error Writing hit to log...");
            }
        }
        Log.m3v(httpHeaders.toString());
    }
    ...
    URL getUrl(Hit hit) {
        ...
        try {
            ...
            Log.m1e("Error trying to parse the hardcoded host url. This really shouldn't happen.");
            return null;
        }
    }
