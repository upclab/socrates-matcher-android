...
public class GAServiceManager extends ServiceManager {
        ...
        public boolean handleMessage(Message msg) {
            if (GAServiceManager.MSG_KEY == msg.what && GAServiceManager.MSG_OBJECT.equals(msg.obj)) {
                ...
                if (GAServiceManager.this.dispatchPeriodInSeconds > 0 && !GAServiceManager.this.storeIsEmpty) {
                    GAServiceManager.this.handler.sendMessageDelayed(GAServiceManager.this.handler.obtainMessage(GAServiceManager.MSG_KEY, GAServiceManager.MSG_OBJECT), (long) (GAServiceManager.this.dispatchPeriodInSeconds * DateTimeConstants.MILLIS_PER_SECOND));
                }
            }
            return true;
        }
    ...
    class C04631 implements AnalyticsStoreStateListener {
        C04631() {
        }
    ...
    private void initializeHandler() {
        this.handler = new Handler(this.ctx.getMainLooper(), new C00852());
    ...
    public synchronized void dispatchLocalHits() {
        if (this.thread == null) {
            Log.m3v("Dispatch call queued. Dispatch will run once initialization is complete.");
            this.pendingDispatch = true;
        } else {
    ...
    public synchronized void setLocalDispatchPeriod(int dispatchPeriodInSeconds) {
        if (this.handler == null) {
            Log.m3v("Dispatch period set with null handler. Dispatch will run once initialization is complete.");
    ...
    public void setForceLocalDispatch() {
        if (this.thread == null) {
            Log.m3v("setForceLocalDispatch() queued. It will be called once initialization is complete.");
    ...
    synchronized void updatePowerSaveMode(boolean storeIsEmpty, boolean connected) {
        if (!(this.storeIsEmpty == storeIsEmpty && this.connected == connected)) {
            ...
            Log.m3v(append.append(str).toString());
