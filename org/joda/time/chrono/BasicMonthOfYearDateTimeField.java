...
class BasicMonthOfYearDateTimeField extends ImpreciseDateTimeField {
    ...
    public int[] add(ReadablePartial readablePartial, int i, int[] iArr, int i2) {
        ...
        if (readablePartial.size() > 0 && readablePartial.getFieldType(0).equals(DateTimeFieldType.monthOfYear()) && i == 0) {
            return set(readablePartial, 0, iArr, ((((readablePartial.getValue(0) - 1) + (i2 % 12)) + 12) % 12) + MIN);
        }
    ...
    public long addWrapField(long j, int i) {
        return set(j, FieldUtils.getWrappedValue(get(j), i, MIN, this.iMax));
    }
    ...
    public long set(long j, int i) {
        FieldUtils.verifyValueBounds((DateTimeField) this, i, (int) MIN, this.iMax);
        int year = this.iChronology.getYear(j);
