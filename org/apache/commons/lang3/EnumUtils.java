...
public class EnumUtils {
    ...
    private static <E extends Enum<E>> Class<E> checkBitVectorable(Class<E> enumClass) {
        ...
        Validate.isTrue(z, "%s does not seem to be an Enum type", enumClass);
        if (constants.length <= 64) {
            z = true;
        } else {
            z = false;
        }
        Validate.isTrue(z, "Cannot store %s %s values in %s bits", Integer.valueOf(constants.length), enumClass.getSimpleName(), Integer.valueOf(64));
        return enumClass;
    }
}
